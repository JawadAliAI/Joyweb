 ...
 *
 * Reads `/agent/withdrawals`, scoped server-side to this agent's members. Read-only:
...
export default function AgentWithdrawalsPage() {
  ...
  const query = useQuery({
    queryKey: ['agent', 'withdrawals', params],
    queryFn: () => api.get<AgentMovementPage>('/agent/withdrawals', params),
  });
  ...
  return (
    <>
      ...
      <StatTiles
        ...
        tiles={[
          { label: 'Withdrawals', value: total },
          { label: 'Total withdrawn', value: formatAmount(query.data?.amountTotal, 2, '0') },
